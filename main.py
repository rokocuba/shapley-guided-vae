...
def run_static_joint_experiment(
    ...
    train_loader = DataLoader(
        cast(Dataset[torch.Tensor], train_tensor),
        batch_size=batch_size,
        shuffle=True,
    )
    test_loader = DataLoader(
        cast(Dataset[torch.Tensor], test_tensor),
        batch_size=batch_size,
        shuffle=False,
    )
    history = run_static_joint_training(
        trainer=trainer,
        train_loader=train_loader,
        block_index=block_index,
    ...
    run_dir = save_training_run(
        ...
        output_dir=output_dir,
        training_type="static-joint",
        shapley_tactic="static",
    )
    run_dir = Path(run_dir)
    residual_png = run_dir / "feature_difference_distributions.png"
    residual_summary_csv = run_dir / "feature_difference_summary.csv"
